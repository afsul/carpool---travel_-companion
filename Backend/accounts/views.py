...
class SignupView(generics.GenericAPIView):
    ...
    def post(self, request):
        print('confirm user',request.data)
        data = request.data
...
class LoginView(APIView):             # this is highly required when creating rides (token validation) -- it will work even if url is not provided
    print('afsul is here .........')
    permission_classes = []
    ...
    def post(self, request: Request):
        ...
        if User.objects.filter(email=email).exists():
            ...
            if check_password(password, user.password):
                print(request.user,"This is request user -------iii")
